...
router.get('/buckets', async (req, res, next) => {
    ...
    if (!bucket_name || bucket_name === '#') {
        try {
            ...
            const buckets = await new BucketsApi().getBuckets({ limit: 100, region: 'EMEA'}, req.oauth_client, req.oauth_token);
...
router.post('/buckets', async (req, res, next) => {
    ...
    payload.policyKey = 'persistent'; // no expires (CAMBIO)
    try {
        ...
        await new BucketsApi().createBucket(payload, {xAdsRegion: 'EMEA'}, req.oauth_client, req.oauth_token);
        res.status(200).end();
    } catch (err) {
        next(err);
    }
